...
def IDtoName(stock):
    Raw_Table = pd.read_csv("沪市列表ID&Name_Doing.csv")
...
if __name__ == '__main__':
    Market_DF = pd.read_csv("沪市列表ID&Name_Doing.csv")
    Market_List = []
    ...
    for j in Market_List:
        ...
        Commend_DF_temp.to_csv("ShangHai_Market_11.11/{}.csv".format(Meta_Name), index=False, encoding='utf-8-sig')
        ...
        Market_DF.to_csv("沪市列表ID&Name_Doing.csv", index=False,
                                header=["code", "name"], encoding='utf-8-sig')  # 不保留索引避免多一列索引数据影响运行，然后指定保留列名字为code为了复用代码
        with open("沪市记录.txt", 'a') as f:  # 写个文件记录一下运行的时间数据
